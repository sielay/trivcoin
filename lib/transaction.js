...
const POS_REWARD = 0.1;
const MAX_POS = 1000000;
// const MIN_POST = 1000;
const MIN_POS_AGE = 1000 * 60 * 60 * 24 * 7;

// @see
// https://www.google.co.uk/search?q=casper+algorithm&safe=off&rlz=1C5CHFA_enGB770GB770&source=lnms&tbm=isch&sa=X&ved=0ahUKEwiz_Ni-xq_YAhVLIMAKHWMcDQIQ_AUICigB&biw=1920&bih=1103#imgrc=QRqmijnPcc-ukM:
...
class Transaction {
    ...
    static regular(type, amount, sender, recipient, senderPrivateKey, unspentOutputTransactions) {
        ...
        if (acutalAmount !== undefined && acutalAmount > inputsBalance) {
            throw new Error("Not enough funds");
        }

        if (acutalAmount === undefined) {
            acutalAmount = inputsBalance;
        }

        tx.inputs = unspentOutputTransactions
            .map(t => t.outputs
                .filter(t2 => t2.address === sender)
                .map((i, idx) => Input
                    .generate(i, t.id, idx, senderPrivateKey)))
            .reduce((p, c) => p.concat(c), []);

        if (recipient) {
            tx.outputs = [new Output(recipient, acutalAmount)];

            if (inputsBalance > acutalAmount) {
                tx.outputs.push(new Output(sender, inputsBalance - acutalAmount));
            }
        } else {
            tx.outputs = [];
        }
    ...
    static interest(recipient, stackingTransactions, senderPrivateKey) {
        ...
        tx.inputs = stakes
            .map(t => t.outputs
                .filter(t2 => t2.address === `${recipient}:PoS`)
                .map((i, idx) => Input
                    .generate(i, t.id, idx, senderPrivateKey)))
            .reduce((p, c) => p.concat(c), []);
